...
public class TheBoardInfo : MonoBehaviour
{
	...
	private void UpdateBoard()
	{
		...
		{
			string text = TheDataManager.Instance.GetTotalStarWasUsed(TheEnumManager.STAR.white) + "/" + TheDataManager.THE_PLAYER_DATA.GetTotalStar(TheEnumManager.DIFFICUFT.Normal).ToString();
			this.txtValue.text = text;
			break;
		}
		case TheEnumManager.BOARD_INFO.StarToUpgradeBoard_Hard:
		{
			string text2 = TheDataManager.Instance.GetTotalStarWasUsed(TheEnumManager.STAR.blue) + "/" + TheDataManager.THE_PLAYER_DATA.GetTotalStar(TheEnumManager.DIFFICUFT.Hard).ToString();
			this.txtValue.text = text2;
			break;
		}
		case TheEnumManager.BOARD_INFO.StarToUpgradeBoard_Nightmate:
		{
			string text3 = TheDataManager.Instance.GetTotalStarWasUsed(TheEnumManager.STAR.yellow) + "/" + TheDataManager.THE_PLAYER_DATA.GetTotalStar(TheEnumManager.DIFFICUFT.Nightmate).ToString();
